...
function AverageSessionsChart({ sessions }) {
  ...
  function handleMouseMove(chartState) {
    const activeX = chartState?.activeCoordinate?.x;
    setHoverX(Number.isFinite(activeX) ? activeX : null);
  }

  return (
    <div className="average-sessions-chart">
      {hoverX !== null ? <div className="average-sessions-hover-overlay" style={{ left: hoverX }} /> : null}
      <h3 className="average-sessions-title">Duree moyenne des sessions</h3>
      <div className="average-sessions-content">
        <ResponsiveContainer width="100%" height={200}>
          <LineChart
            data={toChartData(safeSessions)}
            margin={{ top: 16, right: 6, left: 6, bottom: 8 }}
          ...
          >
            ...
            <Tooltip content={<AverageSessionsTooltip />} animationDuration={0} cursor={false} />
            <Line
              type="monotone"
